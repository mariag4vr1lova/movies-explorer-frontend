...
function FilterCheckbox({ isCheck, changeShort, firstEntrance, }) {
        ...
        <label className={`filter__container-label ${firstEntrance && 'filter__container-label_disabled'}`}>
            ...
            <input type="checkbox" className='filter__ckeck' onChange={() => changeShort()} disabled={firstEntrance}/>
            <svg className='filter__check-svg' width="36" height="20" viewBox="0 0 36 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g id="smalltumb">
                <rect
                    className={`filter__check-svg-rect ${!isCheck ? 'filter__check-svg-rect_active' : ''}`}
                    id="tumb__COLOR:tumbler-on" width="36" height="20" rx="10" fill="#2BE080"
                />
                <circle
                    className={`filter__check-svg-circle ${!isCheck ? 'filter__check-svg-circle_active' : ''}`}
